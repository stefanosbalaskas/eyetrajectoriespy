...
| Dynamic time warping trajectory cost | implemented; raw symmetric three-step cumulative cost, no normalization/window/slope constraint | `dynamic_time_warping_cost()` / `pairwise_dynamic_time_warping_costs()` |
...
Versions 0.21–0.33 treat documentation and mathematical metadata as tested package surfaces:
