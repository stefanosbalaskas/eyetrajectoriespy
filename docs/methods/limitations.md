...
## Finite-B precision is not scientific uncertainty

The 0.20 exact binomial intervals concern the Monte Carlo exceedance probability generated by a finite number of retained wild-bootstrap draws. They are not confidence intervals for centered FPCR projections or future outcomes.

An interval that overlaps alpha identifies limited numerical separation from the threshold; it does not prove that the scientific null is true or false. Increasing B can reduce Monte Carlo uncertainty but cannot repair dependence violations, model misspecification, multiplicity assumptions, or adaptive component selection.
