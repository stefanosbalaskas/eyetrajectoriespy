...
The `dimensions` argument is required for the `TrajectorySet` API. The package does not silently include every functional channel, because adding pupil, physiology, or other state dimensions would change the point metric and therefore the path distance.
