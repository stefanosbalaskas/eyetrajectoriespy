...
| How does a repeated binary or count functional response change with trial-varying scalar predictors? | marginal generalized function-on-scalar GEE | participant is the independent cluster; declare Bernoulli/logit or Poisson/log and the coefficient basis; working independence + robust sandwich in 0.51 |
| What marginal probability or expected-count trajectory corresponds to a fixed scalar predictor profile? | generalized FoSR fixed-profile prediction | predeclare profile values; reuse participant bootstrap; flag scalar-range extrapolation; mean-function band, not future-response prediction |
