...
| How does a repeated binary or count functional response change with trial-varying scalar predictors? | marginal generalized function-on-scalar GEE | participant is the independent cluster; declare Bernoulli/grouped-binomial logit or Poisson/log and the coefficient basis; grouped data require integer successes + denominators; Poisson rates require explicit positive exposure; working independence + robust sandwich |
| What marginal probability, exposure-adjusted rate, or expected-count trajectory corresponds to a fixed scalar predictor profile? | generalized FoSR fixed-profile prediction | predeclare profile values; Poisson expected counts from an exposure-adjusted fit require target exposure; reuse participant bootstrap; flag scalar-range extrapolation; mean/rate-function band, not future-response prediction |
