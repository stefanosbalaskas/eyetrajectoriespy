...
_CONTRACTS = (
    ...
    MathematicalContract(
        ...
        equations=(
            r"I_1^{*(b)},\ldots,I_n^{*(b)}\overset{iid}{\sim}"
            r"\{1,\ldots,n\}",
        ...
        scope=(
            "Whole-participant case bootstrap with replacement. Every sampled "
            "occurrence receives a distinct bootstrap group identity, including "
            "duplicate source participants. Fixed effects, random-effect covariance, "
            "and residual variance are refit in every replicate while basis sizes, "
            "knot construction, preprocessing, predictor specification, random-slope "
            "choice, random-effect structure, REML/ML choice, and optimizer remain "
            "fixed. Any failed replicate raises; none are silently discarded or "
            "redrawn. Variance-component bootstrap distributions are retained as "
            "stability diagnostics rather than automatic calibrated intervals."
        ),
    ),
    MathematicalContract(
