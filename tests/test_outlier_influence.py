...
def test_empirical_score_covariance_path_and_fitted_sample_mismatch():
    gaze = atypical_sample()
    ...
    result = diagnose_fpca_outliers(
        ...
        gaze,
        n_components=3,
        score_covariance="empirical",
    )
    assert np.isfinite(result.diagnostics["score_mahalanobis_sq"]).all()

    reordered = gaze.subset(np.arange(gaze.n_curves)[::-1])
    with pytest.raises(ValueError, match="IDs/order"):
        diagnose_fpca_outliers(fit, reordered, n_components=3)


def test_robust_covariance_requires_enough_curves():
    gaze = simulate_planar_trajectories(
        n_participants=2,
        trials_per_participant=2,
        n_time=31,
    ...
    fit = fit_mfpca(gaze, n_components=2)
...
def test_influence_rejects_missing_group_values_and_too_large_omission():
    gaze = simulate_planar_trajectories(
        n_participants=3,
        trials_per_participant=2,
        n_time=31,
    ...
    metadata2 = gaze.metadata.reset_index(drop=True).copy()
    metadata2["block"] = ["A", "A", "A", "A", "B", "B"]
    grouped = TrajectorySet(
        gaze.time,
        gaze.values,
        gaze.curve_ids,
        gaze.dimension_names,
    ...
    with pytest.raises(ValueError, match="too few trajectories"):
        leave_one_group_out_fpca_influence(
            grouped,
            group_column="block",
            n_components=3,
